...
def mapSensor(add, val,i):
	...
	if sensor == "/pot":
		...
		bname = "Button1" if num == 0 else "Button2" if num == 1 else "Button3" if num == 2 else "Button4"
		# updateSwitchVals(num,val)
		note = mappings[bname]['Number']
		if val > 0:
			msg = ump.MIDIMessageCreator.midi2_0_note_on(note, 0, 127)
			midiDAW.convert_midi2_0_to_midi1_0_sm(msg.packet_to_binary())
		else:
			msg = ump.MIDIMessageCreator.midi2_0_note_off(note, 0, 127)
			midiDAW.convert_midi2_0_to_midi1_0_sm(msg.packet_to_binary())
		...
		sendOSC( 'vca', 1, "VCA", state['magnitude']*127)
		#calculate synth params
		# calcVoiceGains()
		# calcLPF()
		# calcLFOs()
		# calcPitchGlide()
		# print ("magnitude", state['magnitude'])

		# print ("jerk", state['jerk'][0])
		# lj[i%1000] = abs(state['jerk'][2])
		# print (max(lj))
		...
		#angle[0], wave plane, -1 pinky, +1 thumb
		#angle[1], vertical striped fold down, -1 down, 0 level filter
		#angle[2], twisty, -1 palm in, +1 palm out
		state['tilt'] = calcTilt(val)

		data = state['angle']

		# turn float data into 32 bit integer
		val0 = data[0]
		val0 = 0 if val0 < -1 else 2 if val0 > 1 else round((val0+1)*50)/50 
		val1 = data[1] * -1 + 0.5
		val1 = 1 if val1 > 1 else 0 if val1 < 0 else round(val1*8)/8
		val2 = data[2]
		val2 = -1 if val2 < -1 else 1 if val2 > 1 else round(val2*50)/50

		# print (val0, val1, val2)
		if val0 != state['cc'][0]:
			num = mappings["Roll"]["Number"]
			msg = ump.MIDIMessageCreator.midi2_0_cc(num, int((val0)/2*127))
			midiDAW.convert_midi2_0_to_midi1_0_sm(msg.packet_to_binary())
			state['cc'][0] = val0
			print (val0)

		if val1 != state['cc'][1]:
			num = mappings["Pitch"]["Number"]
			msg = ump.MIDIMessageCreator.midi2_0_cc(num, int(val1*127))
			midiDAW.convert_midi2_0_to_midi1_0_sm(msg.packet_to_binary())
			state['cc'][1] = val1
			print (val1)

		if val2 != state['cc'][2]:
			num = mappings["Yaw"]["Number"]
			msg = ump.MIDIMessageCreator.midi2_0_cc(num, int((val2+1)*127//2))
			midiDAW.convert_midi2_0_to_midi1_0_sm(msg.packet_to_binary())
			state['cc'][2] = val2
			print (val2)
...
def calcMagnitude(vals):
	...
	return 0 if outVal < 0 else 1 if outVal > 1 else outVal
